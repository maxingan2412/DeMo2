...
class DeMo_Parallel(nn.Module):
    ...
    2. 多样性：9个特征从不同角度捕捉模态信息
    3. 鲁棒性：DGAF 分支可自适应调整权重
    ...
    def __init__(self, num_classes, cfg, camera_num, view_num, factory):
        ...
        self.BACKBONE = build_transformer(
            num_classes, cfg, camera_num, view_num, factory, feat_dim=self.feat_dim
        )
    ...
    def load_param(self, trained_path):
        """加载预训练权重"""
        state_dict = torch.load(trained_path, map_location="cpu")
        print(f"Successfully load ckpt from {trained_path}")
        incompatibleKeys = self.load_state_dict(state_dict, strict=False)
        print(incompatibleKeys)
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None):
        ...
        Args:
            x: dict with keys 'RGB', 'NI', 'TI'
            label: class labels (training only)
            cam_label: camera labels
            view_label: view labels

        Returns:
            Training: (score1, feat1, score2, feat2, ..., score9, feat9) - 18个值
            Inference: cat([feat1, ..., feat9]) - (B, 9C)
        """
        ...
        # ========== Helper Function ==========
        def fuse_global_local(feat_cash, feat_global, pool_layer, reduce_layer):
            """Global-Local Fusion: pool(tokens) + concat(global)"""
            feat_local = pool_layer(feat_cash.permute(0, 2, 1)).squeeze(-1)
            return reduce_layer(torch.cat([feat_global, feat_local], dim=-1))
...
def make_model(cfg, num_class, camera_num, view_num=0):
    ...
    根据配置选择模型架构：
    - DeMo: 简化版（SDTPS/DGAF/Baseline）
    - DeMo_Parallel: 并行架构（9个分类头）
    - DeMoBeiyong: 完整版（包含SACR/LIF/HDM/ATM）
    """
    model_arch = cfg.MODEL.ARCH if hasattr(cfg.MODEL, 'ARCH') else 'DeMo'
    ...
    if model_arch == 'DeMo_Parallel':
        ...
        print('===========Building DeMoBeiyong (Full Version)===========')
    else:
        model = DeMo(num_class, cfg, camera_num, view_num, __factory_T_type)
        print('===========Building DeMo (Simplified)===========')
